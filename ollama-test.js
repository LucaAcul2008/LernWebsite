...
// // Import Cerebras SDK using proper Node.js syntax
// const { Cerebras } = require('@cerebras/cerebras_cloud_sdk');

// const client = new Cerebras({
//   apiKey: process.env['csk-ffxtrfecp63c38t9j8422mc48x2r8mc8dpf2rfekrpep4xek'], // This is the default and can be omitted
// });
...
// // API endpoints
// const OLLAMA_API = process.env.OLLAMA_API || 'http://localhost:11434/api';
// // Fixed the typo in the environment variable name
// const CELEBRAS_API = process.env.CELEBRAS_API || 'http://localhost:9000/api/v1/chat/completions';
...
// // Call Celebras API with retry mechanism
// async function callCelebrasAPIWithRetry(prompt, action, retries = 0) {
//   const requestId = uuidv4();
//   try {
//     console.log(`[${new Date().toISOString()}] [${requestId}] 🤖 Using Celebras API for ${action}`);
    
//     // Prepare headers with API key
//     const headers = {
//       'Content-Type': 'application/json',
//       'Authorization': `Bearer ${process.env.CELEBRAS_API_KEY}`
//     };
    
//     // Format the prompt for Celebras API
//     const response = await axios.post(CELEBRAS_API, {
//       prompt: prompt,
//       stream: false
//     }, { 
//       headers,
...
//     if (!message || !message.content) {
//       console.warn(`[${new Date().toISOString()}] [${requestId}] ⚠️ Invalid or empty response from Celebras API`);
//       return { response: "Sorry, I couldn't generate a response." };
//     }
    
//     console.log(`[${new Date().toISOString()}] [${requestId}] ✅ Received valid response from Celebras API`);
//     return { response: message.content };
//   } catch (error) {
//     console.error(`[${new Date().toISOString()}] [${requestId}] ❌ Error calling Celebras API (attempt ${retries + 1}/${MAX_RETRIES + 1}):`, error.message);
...
//       console.log(`[${new Date().toISOString()}] [${requestId}] 🔄 Retrying in ${delay/1000} seconds...`);
//       await new Promise(resolve => setTimeout(resolve, delay));
//       return callCelebrasAPIWithRetry(prompt, action, retries + 1);
//     }
...
// // Process AI prompt with Celebras API
// async function processPrompt(prompt, action, retries = 0) {
//   const requestId = uuidv4();
//   // Set this to true to use Celebras API
//   const useCelebrasAPI = true;
  
//   console.log(`[${new Date().toISOString()}] [${requestId}] 📝 Processing prompt for action: ${action}, API: ${useCelebrasAPI ? 'Celebras' : 'Ollama'}`);
  
//   try {
//     if (useCelebrasAPI) {
//       return await callCelebrasAPIWithRetry(prompt, action, retries);
//     }
//   } catch (error) {
//     console.error(`[${new Date().toISOString()}] [${requestId}] ❌ Error in processPrompt:`, error.message);
//     throw new Error(`Failed to process prompt: ${error.message}`);
...
// // Handle requests to our AI service
// app.post('/api/ai', validateApiRequest, async (req, res) => {
//   try {
//     const { action, message, context, material, exam, materials } = req.body;
...
// // Fix the API key initialization - use a direct string instead of trying to access an environment variable
// const client = new Cerebras({
...
//     console.log('Cerebras API Test Response:');
//     console.log(completionCreateResponse);
//     return completionCreateResponse;
//   } catch (error) {
//     console.error('Error testing Cerebras API:', error.message);
//     throw error;
//   }
// }
...
//     // Use the globally initialized client instead of making separate requests
...
//       case 'summarize': 
//       // ... existing code for summarize case
      
//       case 'generateQuiz':
//       // ... existing code for generateQuiz case
      
//       case 'generateStudyPlan':
//       // ... existing code for generateStudyPlan case
      
//       case 'getStudyTips':
//       // ... existing code for getStudyTips case
...
// // Handle requests to our AI service - primary endpoint
// app.post('/api/ai', validateApiRequest, async (req, res) => {
//   try {
//     const { action, message, context, material, exam, materials } = req.body;
